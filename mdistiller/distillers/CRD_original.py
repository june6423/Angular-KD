        ...
        if self.cfg.DIV.USAGE:
            ...
            for i in range(self.cfg.TEKAP.AUGNUM):
                ...
                if self.cfg.TEKAP.LOGIT:
                    ...
                    logit_noise = F.softmax(logit_noise / self.cfg.TEKAP.T, dim=1)
                    
                    loss_ce += self.ce_loss_weight * 0.8* (F.kl_div(p_s, logit_noise, size_average=False) * (self.cfg.TEKAP.T**2) / logits_student.shape[0])
