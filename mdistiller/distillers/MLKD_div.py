...
class MLKD_div():
    ...
    def mlkd_loss(self, logits_student_weak, logits_student_strong, logits_teacher_weak_dict, logits_teacher_strong_dict , target, **kwargs):
        batch_size, class_num = logits_student_strong.shape

        pred_teacher_weak = logits_teacher_weak_dict[1.0].detach()
        ...
        loss_kd_weak = self.kd_loss_weight * ((kd_loss(
            logits_student_weak,
            logits_teacher_weak_dict[self.temperature],
            self.temperature,
            # reduce=False
            logit_stand=self.logit_stand,
        ) * mask).mean()) + self.kd_loss_weight * ((kd_loss(
            logits_student_weak,
            logits_teacher_weak_dict[3.0],
            3.0,
            # reduce=False
            logit_stand=self.logit_stand,
        ) * mask).mean()) + self.kd_loss_weight * ((kd_loss(
            logits_student_weak,
            logits_teacher_weak_dict[5.0],
            5.0,
            # reduce=False
            logit_stand=self.logit_stand,
        ) * mask).mean()) + self.kd_loss_weight * ((kd_loss(
            logits_student_weak,
            logits_teacher_weak_dict[2.0],
            2.0,
            # reduce=False
            logit_stand=self.logit_stand,
        ) * mask).mean()) + self.kd_loss_weight * ((kd_loss(
            logits_student_weak,
            logits_teacher_weak_dict[6.0],
            6.0,
            # reduce=False
            logit_stand=self.logit_stand,
        ) * mask).mean())
        ...
        loss_kd_strong = self.kd_loss_weight * kd_loss(
            ...
            logits_teacher_strong_dict[self.temperature],
            self.temperature,
            logit_stand=self.logit_stand,
        ) + self.kd_loss_weight * kd_loss(
            logits_student_strong,
            logits_teacher_strong_dict[3.0],
            3.0,
            logit_stand=self.logit_stand,
        ) + self.kd_loss_weight * kd_loss(
            logits_student_strong,
            logits_teacher_strong_dict[5.0],
            5.0,
            logit_stand=self.logit_stand,
        ) + self.kd_loss_weight * kd_loss(
            logits_student_weak,
        ...
        loss_bc_strong = self.kd_loss_weight * ((bc_loss(
            ...
            logits_student_strong,
            logits_teacher_strong_dict[2.0],
            2.0,
        ) * mask).mean()) + self.kd_loss_weight * ((bc_loss(
            logits_student_strong,
            logits_teacher_strong_dict[6.0],
            6.0,
        ) * mask).mean())
        losses_dict = {
            "loss_ce": loss_ce,
