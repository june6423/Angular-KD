...
class CRD_mlkd(Distiller):
    """Contrastive Representation Distillation"""

    def __init__(self, student, teacher, cfg, num_data):
        super(CRD_mlkd, self).__init__(student, teacher)
    ...
    def forward_train(self, image_weak, image_strong, target, index, contrastive_index, **kwargs):
        ...
        assert self.cfg.DIV.MLKD is True
        
        if self.cfg.DIV.USAGE:
            logits_teacher, feature_teacher, loss_dict, logits_teacher_weak_dict = self.teacher(image_weak, loss=True, target=target, various_temp=True)
            _, _, _, logits_teacher_strong_dict = self.teacher(image_strong, target=target, various_temp=True)
            logits_student_strong, _ = self.student(image_strong)
            
            mlkd = MLKD_div(self.cfg)
            _ , loss_mlkd_dict = mlkd.mlkd_loss(logits_student, logits_student_strong, logits_teacher_weak_dict, logits_teacher_strong_dict ,target= target, )                
            ...
            loss_dict = {**loss_mlkd_dict, **loss_dict}
            loss_dict['loss_crd'] = loss_crd
            return logits_student, loss_dict
        
        else:
            raise NotImplementedError("MLKD is not implemented for this case.")
